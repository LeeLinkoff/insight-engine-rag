...
//
// Works with server.ts as-is:
//   import { injectHighlight } from './highlight-safe'
//
// Behavior:
...
export interface HighlightDiag {
  enabled: boolean;
  mode: 'console' | 'overlay';
}

export interface InjectHighlightOptions {
  baseHref?: string;
  maxGap?: number; // number (tokens); if too small and no match, client retries at 800 automatically
  diag?: HighlightDiag;
}
...
function ensureHead(html: string): string {
  return /<head[\s>]/i.test(html)
    ? html
    : html.replace(/<html[^>]*>/i, (m: string) => `${m}<head></head>`);
}
...
function removeExistingBase(html: string): string {
  return String(html || '').replace(/<base[^>]*>\s*/gi, '');
}

/* ---------- CLIENT (runs inside the proxied page) ---------- */
// This string is injected as a raw <script> into the proxied page and runs
// in the browser, not in this Node process, so it is intentionally plain
// JS text rather than TypeScript. Its internal logic is unchanged from
// highlight-safe.js.
 ...
 * Injects a boot script plus the standalone client highlighter into a raw
 * HTML page so it can locate and mark the given needle text client-side.
 */
export function injectHighlight(html: string, needle: string, opts: InjectHighlightOptions = {}): string {
  const {
    baseHref,
  ...
  if (baseHref) {
    ...
    out = out.replace(/<head[^>]*>/i, (m: string) => `${m}<base href="${baseHref}">`);
  }
