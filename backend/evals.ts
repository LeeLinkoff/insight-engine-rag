...
// Usage
// -----
//   1. Start the server (npm run dev / node dist/server.js)
//   2. Ingest the docs referenced in TEST_CASES below via /api/ingest or /api/ingest-urls
//   3. npx tsx evals.ts   (or compile first, then node dist/evals.js)
//
...
type ExpectedOutcome = 'answer' | 'refusal';
...
interface QueryResponse {
  ...
  source_diversity?: { unique_sources: number; total_chunks_used: number; single_source_warning: boolean };
  sources?: QuerySource[];
}
...
const TEST_CASES: TestCase[] = [
  {
    ...
    // If the URL ever changes or the hashing logic in server.ts changes,
